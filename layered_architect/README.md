# Layered Architecture - 自動販売機システム

このディレクトリは、**層状アーキテクチャ（Layered Architecture）** で実装された自動販売機システムです。

## 概要

層状アーキテクチャは、ビジネスロジックを複数の層に分離し、各層が明確な責務を持つ設計パターンです。本実装では、以下の4層に分離しています：

```
┌─────────────────────────────────────┐
│   Presentation Layer (プレゼン層)    │ ConsoleUI
├─────────────────────────────────────┤
│   Application Layer (アプリ層)       │ Use Cases, Facade
├─────────────────────────────────────┤
│   Domain Layer (ドメイン層)          │ Entities, Value Objects, Services
├─────────────────────────────────────┤
│   Infrastructure Layer (インフラ層)  │ Hardware Adapters, Repository
└─────────────────────────────────────┘
```
...
- **Application層**: ユースケース実装、ビジネス手続きの調整
- **Domain層**: ビジネスロジック、インターフェース定義
- **Infrastructure層**: 外部システムの具体的な実装

**依存関係の原則：**
- Infrastructure層がインターフェースを定義
- Infrastructure層がインターフェースを実装
- Domain層はInfrastructure層のインターフェースに依存
- 依存関係の逆転原則（DIP）は適用せず、上位レイヤーが下位レイヤーに直接依存する
...
│   │   └── ConsoleUI.cpp                   # UI実装
│   ├── application/
│   │   ├── VendingMachineApplication.hpp   # Facade（DI Container）
│   │   ├── VendingMachineApplication.cpp
│   │   ├── repositories/
│   │   │   └── ITransactionHistoryRepository.hpp  # 永続化インターフェース
│   │   └── usecases/
│   │       ├── PurchaseWithCashUseCase.hpp ✅ イベントストーミング対応
│   │       ├── PurchaseWithCashUseCase.cpp
│   │       ├── PurchaseWithEMoneyUseCase.hpp ✅ イベントストーミング対応
...
│   └── infrastructure/
│       ├── interfaces/
│       │   ├── ICoinMech.hpp               # コインメックインターフェース
│       │   ├── IDispenser.hpp              # ディスペンサーインターフェース
│       │   └── IPaymentGateway.hpp         # 決済ゲートウェイインターフェース
│       ├── adapters/
│       │   ├── SimulatedCoinMech.hpp       # コインメック実装
│       │   ├── SimulatedDispenser.hpp      # ディスペンサー実装
│       │   └── SimulatedPaymentGateway.hpp # 決済ゲートウェイ実装
│       └── repositories/
│           └── InMemoryTransactionHistoryRepository.hpp  # メモリベース永続化
├── test/                                    # テストスイート
...
**重要**: Application層が `ICoinMech` インターフェースを定義し、Infrastructure層が `SimulatedCoinMech` の具体的な実装を提供します。Domain層はビジネスロジックのみに専念します。
...
このLayered Architecture実装は、DIP（依存性逆転の原則）を適用した設計です：
