...
public:
   ...
   * @return 在庫がある商品のDTOリスト
   *
   * @note 電子決済では残高チェック不要（決済時に外部で確認される）
   */
  std::vector<dto::ProductDto> getAvailableProducts() const;
   ...
   */
  dto::EMoneyPurchaseResponse
  selectAndRequestPayment(const dto::EMoneyPurchaseRequest &request);
